...
fn version_from_fname(fname: String) -> String {
    let s: String = fname.clone();
    let ver: Option<Version> = Version::from(s.as_str());
...
fn main() {
    ...
    for file in files {
        ...
        if ver_str.chars().count() > 0 {
            if VersionCompare::compare_to(&github_ver, ver_str.as_str(), &version_compare::CompOp::from_sign(">").unwrap()).unwrap() {
                println!("Update available!");
                println!("GitHub version: {}", github_ver);
                println!("Current version: {}", Version::from(ver_str.as_str()).unwrap().as_str());
            break
            }
        }
    }
